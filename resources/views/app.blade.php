...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" @class(['dark' => ($appearance ?? 'system') == 'dark'])>
    <head>
        ...
        <meta name="description" content="{{ $page['props']['seo']['description'] ?? 'Latest curated news from trusted sources.' }}">

        <!-- Open Graph -->
        <meta property="og:title" content="{{ $page['props']['seo']['title'] ?? config('app.name', 'Laravel') }}">
        <meta property="og:description" content="{{ $page['props']['seo']['description'] ?? 'Stay updated with curated news.' }}">
